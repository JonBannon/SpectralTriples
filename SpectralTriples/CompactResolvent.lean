...
/-! # Spectral triples with compact resolvent

In this file we prove the basic criterion of self-adjointness (`Im z ≠ 0 → z ∈ ρ(D)`) and use
it to add the compact-resolvent axiom to the core odd-spectral-triple predicate.

Compact resolvent is part of the usual definition of a spectral triple. It is strictly weaker
than finite (or `p`-)summability, which additionally requires Schatten/trace decay of the
resolvent. This file therefore uses the mathematically precise compact-resolvent terminology;
the deprecated `IsFinitelySummableSpectralTriple` name is retained as a compatibility alias.

## Main definitions

* `IsCompactResolventSpectralTriple`: an odd spectral triple whose Dirac operator has compact
...
* `IsCompactResolventSpectralTriple.resolvent_mem`: field asserting `RCLike.I ∈ ρ(D)`.
...
/-- An odd spectral-triple core has compact resolvent if `RCLike.I` lies in the resolvent set
of the Dirac operator and the resolvent `R(i, D)` is a compact operator on `H`.
...
by `IsOddSpectralTriple.toIsCompactResolventSpectralTriple` via
`IsSelfAdjoint.mem_resolventSet`. -/
structure IsCompactResolventSpectralTriple (A : Type*) {H 𝕜 : Type*} [RCLike 𝕜] [Semiring A]
...
/-- Smart constructor for `IsCompactResolventSpectralTriple`.

Compared with building the structure directly, this def does two things:
* enables dot-notation `hT.toIsCompactResolventSpectralTriple hI hc`;
...
def toIsCompactResolventSpectralTriple (hT : IsOddSpectralTriple A D π)
    (hI : RCLike.im (RCLike.I : 𝕜) ≠ 0)
    (hc : IsCompactOperator (D.resolvent RCLike.I)) :
    IsCompactResolventSpectralTriple A D π where
...
/-- Compatibility alias for the former, mathematically inaccurate name. Compact resolvent alone
does not imply finite/`p`-summability. -/
...
/-- Compatibility alias for `toIsCompactResolventSpectralTriple`. -/
@[deprecated toIsCompactResolventSpectralTriple (since := "2026-08-20")]
abbrev toIsFinitelySummableSpectralTriple (hT : IsOddSpectralTriple A D π)
    (hI : RCLike.im (RCLike.I : 𝕜) ≠ 0)
    (hc : IsCompactOperator (D.resolvent RCLike.I)) :
    IsCompactResolventSpectralTriple A D π :=
  hT.toIsCompactResolventSpectralTriple hI hc
