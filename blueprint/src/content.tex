...
spin-geometry and Sobolev-space machinery from the literature. The two-torus example
(Section~\ref{sec:torus}) is a complete, finitely summable, even spectral triple with
vanishing index; the circle example (Section~\ref{sec:circle}) is a simpler, partially-built
precursor sharing the same diagonal-operator technology (Section~\ref{sec:diagonal-operators}).
...
\textbf{Status: in progress.} The Hilbert space, the diagonal Dirac operator, its
self-adjointness, and membership of $i$ in its resolvent set are formalized and
\texttt{sorry}-free; the compact-resolvent estimate and the algebra representation remain to
be built (shared technology with Section~\ref{sec:torus}, where they are complete).
